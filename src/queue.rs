...
impl TorrentState {
    pub fn new(info: TorrentInfo, peer_list: &PeerList) -> Self {
        ...
        let file_len = info.info_data.length;
        let piece_size = info.info_data.piece_length;
        let bitfield_len: usize = if file_len % piece_size == 0 {
            (info.info_data.length / info.info_data.piece_length / 8) as usize
        } else {
            (info.info_data.length / info.info_data.piece_length / 8) as usize + 1
        };

        let mut bitfield: Vec<u8> = Vec::with_capacity(bitfield_len);
        println!("bitfield len: {}", bitfield_len);
    ...
    /// Set the bitfield on at the given piece index
...
pub async fn start_download(state: TorrentState, client_id: String) {
    ...
    let connections: usize = if total_peers < 5 { total_peers } else { 5 };
    ...
    let mut connection = match PeerConnection::new(ip, port, 3).await {
        Ok(stream) => stream,
        Err(e) => {
    ...
    loop {
        let res = connection.read_from_stream().await;
        if let Err(e) = res {
            eprintln!("{}", e);
            break;
        }

        let response = res.expect("Unexpected error");
        ...
        println!("Response received!! {}", response.len());
                    ...
                    match msg_id {
                        ...
                        MessageId::Piece => {
                            if let Some(payload) = msg.payload {
                                println!("We got a payload {} woohoo!!", payload.len());
                            }
                        }
                        _ => {
...
mod tests {
    ...
    fn bitfield_set() {
        ...
        torrent_state.set_bitfield_on(22);
        assert_eq!(torrent_state.bitfield[2], 0x02);
        assert!(!torrent_state.check_piece(23));
        torrent_state.set_bitfield_on(8);
        assert_eq!(torrent_state.bitfield[1], 0x80);
        assert!(!torrent_state.check_piece(23));
        assert_eq!(torrent_state.get_next_required_piece_index(), Some(0));
        torrent_state.set_bitfield_on(0);
        assert_eq!(torrent_state.get_next_required_piece_index(), Some(1));
    }
}
